...
cf1:{(h x):@[h x;(key h x)0;:;enlist(c;sum (f0[c]-f0[0])xexp 2)]; (h x):@[h x;c;:;enlist((key h x)0;sum (f0[c]-f0[0])xexp 2)]};

cfg:{ gnn[x]; ug[x]; }

ap:{$[1=count h x;cf1[x];cfg[x]]};;

p1:{$[0=count h x;[(h x):()!();];$[1=count h x;.k.ep:key h x;[.k.ep:(raze (h x).k.ep)[0][0]]]];show .k.ep;}
