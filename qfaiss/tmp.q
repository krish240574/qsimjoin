...
/ Helper functions
cf1:{show "1=count h x";rukcf1;(h x):@[h x;(key h x)0;:;enlist(c;sum (f0[c]-f0[0])xexp 2)]; (h x):@[h x;c;:;enlist((key h x)0;sum (f0[c]-f0[0])xexp 2)];.k.ep:0;};
cfg:{ gnn[x]; ug[x]; }
/ Add points 
ap:{$[1=count h x;cf1[x];cfg[x]]};;
/ Phase 1 of construction
p1:{$[0=count h x;[(h x):()!();.k.ep:0];$[1=count h x;.k.ep:key h x;[.k.ep:first(raze (h x).k.ep)[0]]]];show .k.ep;}
