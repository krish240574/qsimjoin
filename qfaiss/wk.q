...
wk:{[v]
  t:{
		...
		(`$":f",string 0) 1: (.k.f);};
	.k.nw:v`nw; .k.lc:(); .k.rd:();.k.hl:();.k.gc:{show "gc";show count .k.f;.k.cc:.k.f x;:.k.cc};.k.ed:();.k.d:v`d;
	...
	.k.ff:{
		...
		show "count x";	
		show count x;
		$[0=count .k.ed;[show "0=#.k.ed";.k.ed:.k.edf[.k.f;x]];[show "0<>#.k.ed";.k.ed:.k.ed,'.k.edf[.k.f;last x]]];
		show "count .k.ed 0";show count .k.ed 0;
		$[1=count x 0;[show "1=#x 0";.k.w:(where(max min each .k.ed)=.k.ed)0;:(.k.w;.k.ed[.k.w];(1,128)#raze .k.f[.k.w])];
		[.Q.gc[];show "1<>count x 0";w:first each where each (m:min each .k.ed)=.k.ed; mm:m[where each {x=y}[w;]each asc raze distinct w]; 
		tmp:raze mm@'wmm:first each where each (max each mm)=mm; tmp:(wmm where(max tmp)=tmp)0;show "returning...";show (tmp;max mm@'wmm;.k.f tmp);:(tmp;max mm@'wmm;.k.f tmp)]]};
	t[v];
