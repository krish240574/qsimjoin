...
.k.ip:{
	...
	$[.k.ep>0;
		[show"inserting using ep";
			t:(value(h x).k.ep)[0][0]; s:sum each (f0[c]-/:f0 .k.nn:distinct t,raze (value each (h x)t)[;0][;0])xexp 2;
		...
		[show"ep=-1;inserting without";
			$[1=count h x;
				...
				[s:(sum each (f0[c]-/:f0 key h x)xexp 2);i:iasc s;s:asc s;$[3<count s;[s:s til 3;i:i til 3];];
