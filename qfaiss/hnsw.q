...
.k.ep:()
.k.gep:{show "getting ep";$[x<0;:-1;.k.ep:.k.ep,(key h x)where (min s)=s:sum each(f0[c]-/:f0 wk:where not c=key h x)xexp 2]};

.k.ip:{
	show "inserting point";
	show .k.ep;
	.k.ep:where (min s)=s:sum each (f0[c] -/:f0 .k.nn:distinct (t[0][0]),raze (value each ((h x) (t:value (h x).k.ep 0)[0][0]))[;0][;0])xexp 2;
	s:sum each (f0[c]-/:f0 wk:where not c=key h x)xexp 2;$[0=count s;:-1;];
	$[1=count h x;(h x):@[h x;c;,;(enlist c)!enlist ((iasc s);asc $[1<count s;each;] s) ];
