...
.k.ip:{
	...
	$[.k.ep>0;
		/[ruk;show"inserting using ep";.k.ep:where (min s)=s:sum each (f0[c]-/:f0 .k.nn:distinct t,raze (value each ((h x)t:(value(h x).k.ep)[0][0]))[;0][;0])xexp 2];	
		[show"inserting using ep";s:sum each (f0[c]-/:f0 .k.nn:distinct t,raze (value each ((h x)t:(value(h x).k.ep)[0][0]))[;0][;0])xexp 2;.k.ep:first i:iasc s;$[3<count s;s:s til 3;];(h x):@[h x;c;:;(enlist c)!enlist ((iasc s);asc s)]];	
		[show"ep=-1;inserting without";s:(sum each (f0[c]-/:f0 key h x)xexp 2);$[3<count s;s:s til 3;];show "s:";show s;
		$[1=count h x;
